#!/usr/bin/env bash
# next_id.sh — the next free decision or lesson number, read from the ledger, not remembered.
#
...
case "${1:-}" in
  ...
  *) echo "usage: tools/next_id.sh D|L" >&2; exit 2 ;;
esac
...
echo "${pfx}$((max + 1))"
[ -z "$dups" ] || echo "note: already claimed twice in the ledger: $dups(historical ids are allowlisted by the census; a new one is a red)" >&2
