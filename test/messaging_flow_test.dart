...
void main() {
  ...
  group('ThreadScreen', () {
    ...
    testWidgets('a thread whose counterparty is not yet named has no Block', (
      ...
      await tester.tap(_iconButton('Thread actions'));
      await tester.pumpAndSettle();
      // The overflow carries no arcade entry any more (D-322).
      expect(find.text('Challenge or taunt'), findsNothing);
  ...
  group('ThreadScreen — a kv:1: frame is not a surface (D-322)', () {
    Widget screen() => MaterialApp(
      builder: _kvWindow,
      home: ThreadScreen(
        conversationId: 'c1',
    ...
    testWidgets('a challenge frame renders as its readable line — no card, '
        'no Accept, no stake (founder ruling 2026-09-11)', (tester) async {
      MessagingService.threadSinceFn = (_, _) async => delta([
        message(
          'txc',
        ...
        message(
          'txt',
          text: 'gg',
          frame: frameDto(kind: 'taunt', game: '', stake: '', id: 'a1b2c3d4'),
        ),
      ]);
      await tester.pumpWidget(screen());
      await tester.pumpAndSettle();
      // The wire's own readable line, as a Kasia user sees it.
      expect(
        find.text('⚔️ Attack & Defend challenge — a duel.'),
        findsOneWidget,
      );
      expect(find.text('gg'), findsOneWidget);
      // Nothing the P2.4 card drew.
      expect(find.text('Attack & Defend'), findsNothing);
      expect(find.text('Accept'), findsNothing);
      expect(find.text('Decline'), findsNothing);
      expect(find.textContaining('Stake'), findsNothing);
    });
  });
