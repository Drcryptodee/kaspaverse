...
/// second.
class _BurialMark extends StatelessWidget {
  const _BurialMark({
    required this.state,
  ...
  Widget build(BuildContext context) {
    ...
    if (n < safe) {
      // The number IS the word. Tabular, so a ticking depth does not jiggle.
      return _Mark(tone: KvLampTone.warn, words: '$n', mono: true);
    }
    if (n < settled) {
      return const _Mark(tone: KvLampTone.ok, words: 'Confirmed');
    }
