...
class TxDetailScreen extends StatelessWidget {
  ...
  Widget _body(BuildContext context) {
    ...
    final name = counterparty == null ? null : contacts?.nameFor(counterparty);
                ...
                Entrance(
                  ...
                  child: KvSurface(
                    ...
                    child: Column(
                      ...
                      children: [
                        // **`Sent` / `Received` / `Mined`, and the verb is the
                        // record's own** (`kvActivityFace`), so the plate opens
                        // by naming what happened rather than repeating a
                        // generic `Time`.
                        //
                        // It states what is true of BOTH clocks behind it
                        // without claiming either. `ActivityRecord.unixtimeMsec`
                        // is wallet-core's `TransactionRecord::unixtime_msec` —
                        // for a row observed live it is the WALLET's clock at
                        // the moment it recorded the transaction, and for a
                        // discovered row it is the node's
                        // `get_daa_score_timestamp_estimate`. Neither is the
                        // accepting block's header timestamp; the ceremony's
                        // `Accepted` line has that one and this field does not
                        // borrow the word.
                        KvFactLine(
                          label: title,
                          valueText: at == null ? '—' : _stamp(at),
                          value: _Value(at == null ? '—' : _stamp(at)),
                        ),
                        if (counterparty != null) ...[
                          ...
                          _CounterpartyLine(
                            ...
                            name: name,
                            onCopy: () =>
                                _copy(context, counterparty, 'Address copied'),
                          ),
                        ],
                        if (record.feeSompi case final fee?) ...[
                          const _Rule(),
                          // Trailing zeros trimmed, like every other figure in
                          // the app since D-267: `0.00010000` reads `0.0001`.
                          _FeeLine(sompi: fee),
                        ],
                        const _Rule(),
                        // **The accepting score when the DAG has one, the
                        // containing block's otherwise.** This is the number
                        // the depth is measured against — `KvBurial.depthOf`
                        // subtracts exactly this from the virtual DAA score —
                        // so putting it on the screen lets a reader check the
                        // gauge's arithmetic instead of trusting it.
                        KvFactLine(
                          label: 'DAA score',
                        ...
                        const _Rule(),
                        // **The id belongs IN the table, not in a well of its
                        // own.** It is one more fact about this transaction,
                        // and giving it a card made the screen read as two
                        // unrelated regions. The tap copies all 64 characters —
                        // a truncated txid is as useless as a truncated address.
                        _IdLine(
                          txid: record.txid,
                          onCopy: () => _copy(
                            context,
  ...
  static String _stamp(BigInt at) =>
  ...
  Widget build(BuildContext context) {
    return KvSurface(
      ...
      child: Column(
        ...
        children: [
          KvBurialGauge(
            // **The axis, named, on the reading's own row** (BG-22). `S9` sets
            // `DEPTH` at the plate's left edge — measured at 8.25 dp of cap,
            // which is 11 dp of Jakarta at its 0.773 ratio, the `caps` role
            // exactly — with the count hard right on the same line.
            heading: const KvRuledLabel('Depth'),
            state: gateByDepth(
              chipStateOf(record.maturity, stalled: record.stalled),
              depth,
            ),
          ...
          // reason is knowable here — it is the same bit the money plate folds
          // into its trust line (BG-11/BG-20).
          //
          // **Its slot is always reserved and only the ink fades** — the same
          // mechanism Receive's caption uses. Built as a bare conditional it
          // cut in with no transition AND took everything below it down the
          // screen in one frame, every time the link flapped, on a surface
          // whose whole purpose is watching a number that is not moving
          // (BG-24; measured, `ux-auditor`, UX-5).
          const SizedBox(height: KvSpace.s),
          AnimatedOpacity(
            opacity: stale ? 1 : 0,
...
/// The `To` row: the name where there is one, and **always** the address it
/// stands for (BG-15).
class _CounterpartyLine extends StatelessWidget {
  ...
  /// Unused for rendering — the row prints the address and the chip prints the
  /// name (BG-19). Kept in the signature so the seat is visible to the next
  /// reader rather than looking like it was never wired.
  final String? name;
  ...
  Widget build(BuildContext context) {
    return KvFactLine(
      ...
      // it cost exactly that. Found in a rendered frame, not in a test.
      label: 'To',
        ...
        child: InkWell(
          ...
          child: _Target(
            child: Column(
              ...
              mainAxisSize: MainAxisSize.min,
              children: [
                // **The address alone, and the name only in the chip above**
                // (`S9`: the `To` row is `kaspa: qr7m … gfx9t` and a copy glyph,
                // no name). Rendering both put the name twice above the fold,
                // which is BG-19, and the address is this row's whole added job —
                // it is what the chip's name stands for, and what BG-15 says must
                // be visible whether or not the book knows a name.
                KvAddress(address, form: KvAddressForm.compact),
              ],
            ),
          ),
        ),
